...
@Mod(modid = E4steamClient.MOD_ID, name = "e4steam", version = "0.2.3", clientSideOnly = true)
public final class E4steamClient {
    ...
    public void clientTick(TickEvent.ClientTickEvent event) {
        ...
        int port = publishedPort(server);
        if (port <= 0) {
            stopSharing();
        } else if (session == null || hostedPort != port) {
            stopSharing();
            hostedPort = port;
    ...
    @SubscribeEvent public void clientCommand(ClientChatEvent event) {
        if (!"/e4steam invite".equalsIgnoreCase(event.getMessage().trim())) return;
        event.setCanceled(true);
        openInvite();
    }

    private static int publishedPort(IntegratedServer server) {
        if (server == null) return 0;
        return server.getPublic() ? server.getServerPort() : 0;
    }
    ...
    public static void sessionReady(final SteamSession ready) {
        Minecraft.getMinecraft().addScheduledTask(new Runnable() {
            @Override public void run() {
                ...
                TextComponentString invite = new TextComponentString("[Invite friends]");
                invite.getStyle().setColor(TextFormatting.AQUA).setClickEvent(
                        new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/e4steam invite")
                );
                Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(invite);
            }
        });
    }
    ...
    private static void openInvite() {
        SteamSession current=session;
        if(current==null||current.state()!=SteamSession.State.STARTED){chat("e4steam: Steam lobby is not ready");return;}
    ...
    public static void acceptSteamInvite(final String endpoint, final String hostName) {
        ...
        if (!parsed.isPresent()) { showSteamJoinFailure("Invalid Steam address"); return; }
                ...
                Minecraft.getMinecraft().addScheduledTask(new Runnable() {
                    @Override public void run() {
                        ...
                        minecraft.displayGuiScreen(new GuiConnecting(
                                ...
                                new ServerData(hostName, address, false)
                        ));
                    }
                });
            } catch (Throwable throwable) { showSteamJoinFailure(throwable.getMessage()); }
        });
    }
    ...
    private static void chat(String message) {
        Minecraft minecraft = Minecraft.getMinecraft();
        if (minecraft.ingameGUI != null) minecraft.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(message));
    }
}
