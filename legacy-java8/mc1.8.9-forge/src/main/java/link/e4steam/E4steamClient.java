...
public final class E4steamClient {
    ...
    @Mod.EventHandler public void init(FMLInitializationEvent event){SteamRuntime.preloadCompatibilityClasses();MinecraftForge.EVENT_BUS.register(this);ClientCommandHandler.instance.registerCommand(new InviteCommand());}
    ...
    @SubscribeEvent public void clientTick(TickEvent.ClientTickEvent event){
        ...
        if(port<=0)stopSharing();
        else if(session==null||hostedPort!=port){stopSharing();hostedPort=port;SteamSession created=new SteamSession(port,SteamAccessMode.FRIENDS_ONLY);session=created;created.startAsync();}
    }
    ...
    public static void sessionFailed(final Throwable throwable){LOGGER.error("Could not start e4steam",throwable);Minecraft.getMinecraft().addScheduledTask(new Runnable(){@Override public void run(){chat("e4steam: "+throwable.getMessage());}});}
    ...
    public static void acceptSteamInvite(final String endpoint,final String hostName){
        ...
        claim.whenComplete((accepted,failure)->{if(failure!=null||!Boolean.TRUE.equals(accepted)){showSteamJoinFailure(failure==null?"Steam invitation rejected":failure.getMessage());return;}try{final InetSocketAddress local=SteamClientBridge.open(parsed.get());Minecraft.getMinecraft().addScheduledTask(new Runnable(){@Override public void run(){Minecraft minecraft=Minecraft.getMinecraft();String address=local.getAddress().getHostAddress()+":"+local.getPort();minecraft.displayGuiScreen(new GuiConnecting(new GuiMultiplayer(new GuiMainMenu()),minecraft,new ServerData(hostName,address,false)));}});}catch(Throwable throwable){showSteamJoinFailure(throwable.getMessage());}});
    }
    public static void showSteamJoinFailure(Object detail){final String message=String.valueOf(detail);LOGGER.warn("Steam join failed: {}",message);Minecraft.getMinecraft().addScheduledTask(new Runnable(){@Override public void run(){chat("e4steam: "+message);}});}
