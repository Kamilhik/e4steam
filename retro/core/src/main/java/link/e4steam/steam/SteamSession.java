...
public final class SteamSession {
    ...
    public SteamSession(int localPort, SteamAccessMode accessMode) {
        if (localPort < 1 || localPort > 65535) {
            throw new IllegalArgumentException("Invalid LAN port");
        }
    ...
    private void start() {
        try {
            ...
            SteamAddress provisional = new SteamAddress(
                    Long.parseUnsignedLong(runtime.steamId()), token);
    ...
    void runtimeFailed(Throwable failure) {
        ...
        E4steamClient.sessionFailed(failure);
        stop();
    }
